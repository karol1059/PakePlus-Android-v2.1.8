...
window.confirmShopCount = function() {
    ...
    for (let i = 0; i < shopCount; i++) {
        const newShopId = gameData.shops.length + 2;
        const shopLevelName = getShopLevelName(params.shopLevel);
        
        // 优化店铺名称显示，1.2.3.号店省略显示
        let shopName;
        if (newShopId <= 3) {
            shopName = `${newShopId}号店`;
        } else {
            shopName = `${newShopId}号${shopLevelName}`;
        }
        
        const newShop = {
            id: newShopId,
            level: params.shopLevel,
            name: shopName,
            status: '建设中',
            openingDay: gameData.expenseCounters.days + 90,
            staffLevel: params.staffLevel,
            staffCount: getStaffCount(params.shop, params.staffLevel),
            rent: params.shop.rent,
            equipment: params.shop.equipment,
            people: params.shop.people
        };
        
        gameData.shops.push(newShop);
        gameData.shopCount++;
        gameData.totalShopCount = (gameData.totalShopCount || 1) + 1;
        
        // 更新总设备价值
        gameData.equipment.totalValue += params.shop.equipment;
        gameData.equipment.currentValue += params.shop.equipment;
        ...
        if (existingBatch) {
            ...
            existingBatch.count += projectorCount;
        } else {
            // 如果当前年份没有批次，创建新批次
            // 限制总批次数量为10个
        ...
        gameData.equipment['放映机'] += projectorCount;
    }
...
function updateStaffExpense(staffCount) {
    // 计算总支出
    const monthlySalaryPerPerson = 5000; // 平均每人月薪
