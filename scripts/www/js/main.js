...
function saveToSlot(slot) {
    ...
    try {
        ...
        console.log('Data serialized, length:', serializedData.length);
        
        // 检查本地存储容量
        if (serializedData.length > 4 * 1024 * 1024) { // 4MB 限制
            throw new Error('存档数据过大，请清理一些历史数据');
        }
        
        localStorage.setItem(`save-slot-${slot}`, serializedData);
        ...
        if (notificationContent) {
            ...
            if (errorMessage.includes('QuotaExceededError')) {
                errorMessage = '存储空间不足，请清理本地存储或删除旧存档';
            }
...
function setGameSpeed(speed) {
    ...
    showCustomConfirm(`确定要将游戏速度调整为${newSpeed}倍速吗？`, function() {
        ...
        const notificationContent = document.getElementById('notification-content');
        if (notificationContent) {
            notificationContent.innerHTML = `<span>游戏速度已调整为${gameSpeed}倍速</span>`;
            checkScrollNeeded();
...
function loadFromSlot(slot) {
    ...
    console.log('Save data length:', saveData ? saveData.length : 0);
    ...
    if (saveData) {
        try {
            ...
            console.log('Game data after load:', gameData);
        ...
        const notificationContent = document.getElementById('notification-content');
        if (notificationContent) {
            notificationContent.innerHTML = `<span>存档不存在：存档位 ${slot} 为空</span>`;
            checkScrollNeeded();
...
function updateSaveSlotInfo() {
    for (let i = 1; i <= 3; i++) {
        const saveData = localStorage.getItem(`save-slot-${i}`);
        ...
        if (slotInfoElement) {
            if (saveData) {
                try {
                    const parsedData = JSON.parse(saveData);
                    const cinemaName = parsedData.gameData.cinemaName || '未命名影院';
                    slotInfoElement.textContent = `${parsedData.timestamp} | ${cinemaName} | 资产: ¥${formatAssets(parsedData.money)} | ${parsedData.date.year}年${parsedData.date.month}月${parsedData.date.day}日`;
                } catch (error) {
...
function confirmRecruitment() {
    ...
    showNotification(`招聘成功：已招聘${recruitCount}名员工，当前配置等级为${levelText}，人员数量为${newStaffCount}人，月支出增加¥${formatAssets(monthlyCostIncrease)}`);
...
function removeFromSchedule(movieId) {
    ...
    if (movie) {
        ...
        // 显示排片移除通知
        showNotification(`排片设置：《${movie.name}》已从排片中移除，返回电影库`);
...
function addToSchedule(movieId) {
    ...
    if (movie) {
        ...
        // 显示添加成功通知
        showNotification(`排片设置：《${movie.name}》已加入排片`);
