...
function closeAdModal(watchAd) {
    ...
    if (watchAd) {
        ...
        setTimeout(() => {
            ...
            showNotification('广告观看完成，本月宣传次数+5！');
...
function updatePromotionDisplay() {
    ...
    // 在宣传方式的 h3 后面显示可用次数
    const promotionSection = document.querySelector('.promotion-section h3');
    if (promotionSection) {
        // 检查是否已经添加了次数显示
        const existingBadge = promotionSection.querySelector('.promotion-count-badge');
        if (existingBadge) {
            // 更新现有徽章
            // 基础次数为1，加上广告获得的额外次数
            const baseCount = 1;
            const totalAvailable = baseCount + gameData.promotions.monthlyPromotionExtra;
            const availableCount = Math.max(0, totalAvailable - gameData.promotions.monthlyPromotionCount);
            existingBadge.textContent = `可用次数: ${availableCount}`;
        } else {
            // 创建新徽章
            const badge = document.createElement('span');
            badge.className = 'promotion-count-badge';
            badge.style.cssText = `
                margin-left: 10px;
                padding: 2px 8px;
            ...
            const totalAvailable = baseCount + gameData.promotions.monthlyPromotionExtra;
            const availableCount = Math.max(0, totalAvailable - gameData.promotions.monthlyPromotionCount);
            badge.textContent = `可用次数: ${availableCount}`;
            promotionSection.appendChild(badge);
        }
    }
...
function updateEquipmentDisplay() {
    ...
    const projectorCount = totalHalls; // 每个厅1台放映设备
...
function simulateDay() {
    ...
    if (gameData.date.day === 1) {
        ...
        if (gameData.shops && gameData.shops.length > 0) {
            gameData.shops.forEach(shop => {
                // 每个分店的基础工资支出
                totalSalaries += 24000; // 假设每个分店的基础工资支出与总店相同
            });
        }
        ...
        const monthlyRecord = {
            ...
            expense: {
                ...
                salaries: gameData.expenses.monthlySalaries,
    ...
    if (gameData.date.day === 1 && gameData.date.month === 1) {
        ...
        let annualExpense = {
            ...
            salaries: 0,
            revenueShare: 0,
            promotion: gameData.promotions.totalPromotionCost,
            total: 0
        };
    ...
    // 今日营收 = 票房收入 + 卖品收入（暂时全额计入，分账在月底扣除）
    const dailyRevenue = totalBoxOffice + totalProductRevenue;
...
function checkStaffBelowMin() {
    ...
    // 基于店铺数量计算必备员工数（考虑班制）
    const totalShops = gameData.shopCount || 1;
    const minStaffCount = totalShops * 3 * workSchedule; // 每个店铺至少3名员工，乘以班制数
    
    // 检查是否低于必备人数
    if (totalStaff < minStaffCount) {
        // 检查是否已经显示过警告
        const hasShownWarning = localStorage.getItem('staffWarningShown') === 'true';
        if (!hasShownWarning) {
            // 显示弹窗提醒
            alert('警告：员工数量低于必备人数，可能会影响影院运营！建议立即招聘员工。');
            
            // 在通知栏显示提醒
            const notificationContent = document.getElementById('notification-content');
            if (notificationContent) {
                notificationContent.innerHTML = `<span>警告：员工数量低于必备人数，可能会影响影院运营！建议立即招聘员工。</span>`;
                checkScrollNeeded();
            ...
            // 标记已经显示过警告
            localStorage.setItem('staffWarningShown', 'true');
        }
    } else {
        // 如果员工数量恢复正常，重置警告标志
        localStorage.removeItem('staffWarningShown');
    }
}
